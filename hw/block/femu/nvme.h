...
#define NVME_CAP_SET_OC(cap, val) \
  (cap |= (uint64_t)(val & CAP_OC_MASK) << CAP_OC_SHIFT)
...
#define NVME_CMBSZ_LISTS(cmbsz) \
  ((cmbsz >> CMBSZ_LISTS_SHIFT) & CMBSZ_LISTS_MASK)
...
typedef struct FemuExtCtrlOps {
  ...
  uint16_t (*io_cmd)(struct FemuCtrl *, NvmeNamespace *, NvmeCmd *,
                     NvmeRequest *);
...
void nvme_set_ctrl_name(FemuCtrl *n, const char *mn, const char *sn,
                        int *dev_id);
...
/* NVMe I/O */
uint16_t nvme_rw(FemuCtrl *n, NvmeNamespace *ns, NvmeCmd *cmd,
                 NvmeRequest *req);
...
#define femu_debug(fmt, ...)                   \
  do {                                         \
    printf("[FEMU] Dbg: " fmt, ##__VA_ARGS__); \
  } while (0)
#else
#define femu_debug(fmt, ...) \
  do {                       \
...
#define femu_err(fmt, ...)                              \
  do {                                                  \
    fprintf(stderr, "[FEMU] Err: " fmt, ##__VA_ARGS__); \
  } while (0)

#define femu_log(fmt, ...)                     \
  do {                                         \
    printf("[FEMU] Log: " fmt, ##__VA_ARGS__); \
  } while (0)
