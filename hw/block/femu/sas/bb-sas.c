...
static void sas_init_ctrl_str(FemuCtrl *n) {
  static int fsid_vbb = 0;
  const char *vbbssd_mn = "FEMU SaS Controller";
...
/* bb <=> black-box */
static void sas_init(FemuCtrl *n, Error **errp) {
...
static uint16_t sas_nvme_rw(FemuCtrl *n, NvmeNamespace *ns, NvmeCmd *cmd,
                            NvmeRequest *req) {
  return nvme_rw(n, ns, cmd, req);
}

static uint16_t sas_io_cmd(FemuCtrl *n, NvmeNamespace *ns, NvmeCmd *cmd,
                           NvmeRequest *req) {
  switch (cmd->opcode) {
    case NVME_CMD_READ:
    case NVME_CMD_WRITE:
      return sas_nvme_rw(n, ns, cmd, req);
    default:
      return NVME_INVALID_OPCODE | NVME_DNR;
  }
}

static uint16_t sas_admin_cmd(FemuCtrl *n, NvmeCmd *cmd) {
  switch (cmd->opcode) {
